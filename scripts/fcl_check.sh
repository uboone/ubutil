...
do
    ...
    cmd="source /cvmfs/uboone.opensciencegrid.org/products/setup_uboone.sh >& /dev/null; setup uboonecode ${UBOONECODE_VERSION} -qe17:prof >& /dev/null; fhicl-dump ${list_test[$i-1]}"
    fix=`ssh -f $(hostname) "${cmd}"`
    out=`diff <(echo "${test}") <(echo "${fix}")`    
