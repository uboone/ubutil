...
void getCalorimetryInformation(TString file1name, TString file1_dataormc, TString file1_label, TString file2name, TString file2_dataormc, TString file2_label, TString outDir, int compType, int isCI, float chisqNotifierCut) {
  ...
  if (isCI == 1){
    ...
    comments = {
      {/*trkdqdx_u_pandoraTrack*/ "trkdqdx_u. Track dQ/dx values on the U (first induction) plane as reconstructed by the pandoraTrack algorithm.",
       /*trkdqdx_v_pandoraTrack*/  "trkdqdx_v. Track dQ/dx values on the V (second induction) plane as reconstructed by the pandoraTrack algorithm.",
       /*trkdqdx_y_pandoraTrack*/  "trkdqdx_y. Track dQ/dx values on the Y (collection) plane as reconstructed by the pandoraTrack algorithm."}
    };

  }
  else {
    // define vector of algo names
    algoNames = {"pandoraNu", "pandoraKalmanTrack", "pmtrack", "pandoraKalmanShower","pandora", "pandoraTrack", "pandoraShower"};
